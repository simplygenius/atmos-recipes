...
data "template_file" "policy-allow-cloudtrail" {
  vars {
    resource = "${replace(aws_cloudwatch_log_group.audit-log.arn, ":*", "")}:log-stream:${lookup(var.account_ids, var.atmos_env)}_CloudTrail_${var.region}*"
  }
...
data "template_file" "policy-allow-s3-cloudtrail" {
  vars {
    ...
    account = "${lookup(var.account_ids, var.atmos_env)}"
  }
