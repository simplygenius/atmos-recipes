...
locals {
  ...
  in_each_downstream_count = "${var.upstream_key == var.current_key && ! contains(var.downstream_keys, var.upstream_key) ? 0 : 1}"
  ...
  in_upstream_either_enablement_count = "${signum(local.in_upstream_aggregate_enablement_count + local.in_upstream_each_key_enablement_count)}"

  upstream_group_names = "${zipmap(var.downstream_keys, formatlist("%s-${var.name}", var.downstream_keys))}"
  downstream_role_names = "${zipmap(var.downstream_keys, formatlist("%s-${var.name}", var.downstream_keys))}"
...
data "template_file" "policy-upstream-allow-assume-downstream-role" {
  ...
  vars {
    account_id = "${lookup(var.account_map, var.downstream_keys[count.index])}"
    role_name = "${var.downstream_keys[count.index]}-${var.name}"
  }

  template = "${file("${path.module}/templates/policy-upstream-allow-assume-downstream-role.tmpl.json")}"
}
...
resource "aws_iam_group_policy" "attach-upstream-allow-assume-downstream-role" {
  ...
  group = "${aws_iam_group.upstream.*.id[count.index]}"
  policy = "${data.template_file.policy-upstream-allow-assume-downstream-role.*.rendered[count.index]}"
}
...
resource "aws_iam_group_policy" "attach-aggregate-upstream-allow-assume-downstream-role" {
  ...
  name = "allow-assume-role-to-${var.downstream_keys[count.index]}-${var.name}"
  group = "${aws_iam_group.upstream-aggregate.id}"
  policy = "${data.template_file.policy-upstream-allow-assume-downstream-role.*.rendered[count.index]}"
}
...
data "template_file" "policy-downstream-allow-upstream-to-assume-role" {
  ...
  template = "${file("${path.module}/templates/policy-downstream-allow-upstream-to-assume-role.tmpl.json")}"
}
...
resource "aws_iam_role_policy" "attach-downstream-role-policy" {
  ...
  role = "${aws_iam_role.downstream.*.name[0]}"

  policy = "${data.template_file.downstream-role-policies.*.rendered[count.index]}"
}
