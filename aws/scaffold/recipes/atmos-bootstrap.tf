...
data "template_file" "policy-allow-assume-env-role-for-bootstrap" {
  ...
  vars = {
    account_id = var.account_ids[var.all_env_names[count.index]]
...
resource "aws_iam_group_policy" "ops-bootstrap-admin" {
  ...
  policy = data.template_file.policy-allow-assume-env-role-for-bootstrap[count.index].rendered
}
