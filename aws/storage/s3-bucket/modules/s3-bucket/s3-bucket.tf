...
EOF
  ...
  logging = slice([local.logging_config], 0, var.logs_bucket == "" ? 0 : 1)
}
...
data "template_file" "bucket-policy" {
  ...
  vars = {
    ...
    enforce_encryption_statements = "${join(
      "",
      data.template_file.bucket-policy-aes-enforcement.*.rendered,
      )}${join(
      "",
      data.template_file.bucket-policy-kms-enforcement.*.rendered,
    )}"
  }
}
